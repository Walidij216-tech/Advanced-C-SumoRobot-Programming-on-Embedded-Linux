...
static_assert(sizeof(io_generic_e) == 2, "Unexpected size, -fshort-enums missing?");
...
// Définition du tableau de registres d'interruption

void configure_interrupts(void) {
     // Configure la détection du front descendant sur P1.0
    P1IES |= BIT0;   // Met le bit de P1IES correspondant à P1.0 à 1
    P1IE  |= BIT0;   // Active l'interruption pour P1.0

    // Configure la détection du front montant sur P2.1
    P2IES &= ~BIT1;  // Met le bit de P2IES correspondant à P2.1 à 0
    P2IE  |= BIT1;   // Active l'interruption pour P2.1
}
 ...
 * exist internally. */
hw_type_e io_detect_hw_type(void)
{
    P1SEL &= ~(BIT4);
    P1SEL2 &= ~(BIT4);
    P1DIR &= ~(BIT4);
    ...
    return P1IN & BIT4 ? HW_TYPE_NSUMO : HW_TYPE_LAUNCHPAD;
}
